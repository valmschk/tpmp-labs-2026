...
## Task 4: Модульное приложение на C
* **Описание:** Программа считывает текст из файла `input.txt`, переводит его в верхний регистр и сохраняет в `output.txt`.
* **Сборка проекта:**
  1. Перейдите в директорию модуля: `cd tpmp-lab2-task4`
  2. Выполните сборку с помощью Makefile: `make`
* **Запуск:** Используйте команду `./main` после успешной сборки.
